...
export class PostgresStatsRepository {
  ...
  async getLeaderboard(gameType?: string, limit: number = 100): Promise<LeaderboardEntry[]> {
    ...
    let query = `
      WITH player_stats AS (
        SELECT 
          ...
          COUNT(*) as total_games,
          SUM(CASE WHEN winner = jsonb_array_elements(state->'players')->>'id' THEN 1 ELSE 0 END) as wins,
          SUM(CASE 
            WHEN winner IS NOT NULL 
            AND winner != jsonb_array_elements(state->'players')->>'id' 
            AND lifecycle = $1 
            THEN 1 
            ELSE 0 
          END) as losses
    ...
    query += `
        GROUP BY jsonb_array_elements(state->'players')->>'id'
        HAVING COUNT(*) >= 5
      )
      SELECT 
        ps.user_id,
